 ...
 * 12/13/2024
 * Punctuation.java
 * This	program	will take some text, read it word by word and print	all	of the	words that have	
 * some	punctuation	in	it.		
 * There will be no field variables.
 * You will	have a variable	for	the	text. You can copy and paste the entire text at the end as	
 * the value of the variable
 * main - calls searchIt
 * searchIt - make the text, call getPunctuationWords, and printWords
 * getPunctuationWords - returns formatted String, words only with punctuation
 * checkForPunctuation - checks for punctuation in the text inputted
 * printWords - prints the words with punctuations
 */
...
public class Punctuation
 {
	 ...
	 public Punctuation()//constructors
	 {
	 }
	 public static void main(String[] args)
	 {
		 Punctuation p = new Punctuation(); 
		 p.searchIt(); // only calling searchIt
	}
	
	public void searchIt()
	{
		String text = new String("\"Blood, Sweat, and Tears by Winston Churchill " + "May 13, 1940" +"Mr. Speaker:" +
...
"and I say, \"Come then, let us go forward together with our united strength.\"");
		String puncWords = getPunctuationWords(text);
		printWords(puncWords);
	}
	...
	public String getPunctuationWords(String text)
	{
		String puncWords = "";
		String words = "";
		for (int i = 0;i<text.length();i++) // for loop to go through text
		{
			char correctWord = text.charAt(i);
			
			if(correctWord != ' '|| i == text.length()-1) // checking if it is not a space
				{
					words+=text.charAt(i);
					
					if(i==text.length()-1)
						{
							if(text.charAt(i)!=' ') // if it is not a space
							{
								puncWords+=words;
								words = "";
							}
			...
			{
				if(checkForPunctuation(words)) // parameter of words
					puncWords+=words + " ";
				words = "";
			}
		}
	...
	public void printWords(String puncWordsIn)
	{
		System.out.print("\n\n\n" + puncWordsIn + "\n\n\n"); // printing all the words that have punctuation
	}
	
	public boolean checkForPunctuation(String spaceIn) // true or false
	{
		String punctuation = new String(".,()!?:;\"'");
		
		for (int i = 0;i<spaceIn.length();i++) // checking words
			{
				for (int j=0;j<punctuation.length();j++) // checking if words has any of the punctuations
				{
					if (spaceIn.charAt(i) == punctuation.charAt(j))
						return true;
				}
			}
			return false;
